...
function clampItem(item, bounds) {
  ...
  let { x, y, vx, vy, width, height } = item;
...
function useContainerRect(containerRef) {
  const [rect, setRect] = useState({ width: 800, height: 900 });
  useEffect(() => {
    const update = () => {
...
function getInitialLayout(containerWidth, heights, aspectRatios) {
...
function getInitialLayoutMobile(containerWidth, heights, aspectRatios) {
  const w = containerWidth || 800;
...
export default function FreeformGallery({ videoSrc, gifSrc, imageSrc, heights: heightsProp }) {
  const heights = heightsProp ?? DEFAULT_HEIGHTS;
  const containerRef = useRef(null);
  const scrollY = useScrollY();
  const { width: cw, height: ch } = useContainerRect(containerRef);
  const boundsRef = useRef({ width: cw, height: ch });

  const isMobile = cw > 0 && cw < MOBILE_BREAKPOINT;
  const [aspectRatios, setAspectRatios] = useState([null, null, null]);
  const [items, setItems] = useState(() =>
    getInitialLayout(typeof window !== "undefined" ? window.innerWidth : 800, heights, [null, null, null])
  );
  ...
  const setAspectRatio = useCallback((index, ratio) => {
  ...
  const updatePosition = useCallback((index, dx, dy) => {
      ...
      {items.map((item, index) => (
        <div
          ...
          style={{
            ...
            width: item.width,
            height: item.height,
            cursor: IS_DESKTOP ? "grab" : "default",
            ...
            <video
              ...
              onLoadedMetadata={(e) => {
                ...
                if (v.videoWidth && v.videoHeight)
                  setAspectRatio(0, v.videoWidth / v.videoHeight);
              }}
